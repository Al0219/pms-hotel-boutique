...
} from "@/modules/services/housekeeping";
...
async function setup(
  service = new MockHousekeepingService(),
  stayService: StayService = new MockStayService(),
  nowMs: () => number = () => morningNowMs,
...
async function ready(ui: Awaited<ReturnType<typeof render>>) {
  await waitFor(() =>
    expect(ui.getByTestId("housekeeping-submit")).toBeTruthy(),
  );
}
...
describe("Housekeeping — IMP-AND-0110", () => {
  it("renders the existing stay, assigned room, confirmed defaults and multiline notes", async () => {
    ...
    expect(
      queryClient.getQueryData(reservationContextKey(activeContext)),
    ).toEqual(expect.objectContaining({ id: currentStayFixture.id }));
    ...
    expect(
      ui.getByTestId("housekeeping-time-picker-options").props.snapToInterval,
    ).toBe(48);
    for (const slot of [
      "09:00–10:00",
      "10:00–11:00",
      ...
      expect(
        ui.getByTestId(`housekeeping-time-picker-option-${slot}`),
      ).toBeTruthy();
    }
  ...
  it("keeps wheel changes temporary until Accept and preserves the field on Cancel", async () => {
    ...
    await fireEvent.press(ui.getByTestId("housekeeping-time-selector"));
    await fireEvent(
      ui.getByTestId("housekeeping-time-picker-options"),
      "momentumScrollEnd",
      {
    ...
    expect(
      within(ui.getByTestId("housekeeping-time-selector")).getByText(
        "09:00–10:00",
      ),
    ).toBeTruthy();
    await fireEvent.press(ui.getByTestId("housekeeping-time-picker-cancel"));
  ...
  it("uses injected nowMs to disable slots inside thirty minutes and blocks a stale selection before submit", async () => {
    ...
    const { ui } = await setup(
      new MockHousekeepingService({ submitRequest }),
      new MockStayService(),
      () => now,
    );
    await ready(ui);
    expect(
      ui.getByTestId("housekeeping-submit").props.accessibilityState.disabled,
    ).toBe(false);
    await fireEvent.press(ui.getByTestId("housekeeping-time-selector"));
    expect(
      ui.getByTestId("housekeeping-time-picker-option-09:00–10:00").props
        .accessibilityState.disabled,
    ).toBe(true);
    expect(
      ui.getByTestId("housekeeping-time-picker-option-10:00–11:00").props
        .accessibilityState.disabled,
    ).toBe(true);
    expect(
      ui.getByTestId("housekeeping-time-picker-option-11:00–12:00").props
        .accessibilityState.disabled,
    ).toBe(false);
    await fireEvent.press(
      ui.getByTestId("housekeeping-time-picker-option-11:00–12:00"),
    );
    await fireEvent.press(ui.getByTestId("housekeeping-time-picker-confirm"));
    expect(
      ui.getByTestId("housekeeping-submit").props.accessibilityState.disabled,
  ...
  it("disables checkout-day slots that cannot finish by the normal checkout policy", async () => {
    const { ui } = await setup(
      ...
      () => new Date(2026, 8, 18, 11, 31).getTime(),
    );
    await ready(ui);
    await waitFor(() =>
      expect(ui.getByTestId("housekeeping-no-availability")).toBeTruthy(),
    );
    expect(
      ui.getByTestId("housekeeping-submit").props.accessibilityState.disabled,
    ).toBe(true);
    ...
    expect(
      ui.getByTestId("housekeeping-time-picker-option-14:00–15:00").props
        .accessibilityState.disabled,
    ).toBe(true);
  });
  ...
  it("selects a temporary QA cleaning type and sends it in the frontend request", async () => {
    ...
    await fireEvent.press(
      ui.getByTestId("housekeeping-type-option-LIGHT_CLEANING"),
    );
    expect(
      ui.getByTestId("housekeeping-type-option-LIGHT_CLEANING").props
        .accessibilityState.selected,
    ).toBe(true);
    expect(
      within(ui.getByTestId("housekeeping-type-selector")).getByText(
        "Limpieza completa",
      ),
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(submitRequest).toHaveBeenCalledWith({
        serviceDate: "2026-09-11",
        timeSlot: "09:00–10:00",
        cleaningType: "LIGHT_CLEANING",
      }),
    );
    await waitFor(() =>
      expect(ui.getByTestId("session-service-requests-probe")).toBeTruthy(),
    );
  });
  ...
  it.each(["", "   \n  "])(
    "omits empty/whitespace notes from the boundary payload (%j)",
    async (notes) => {
      const submitRequest = jest.fn<Promise<void>, [HousekeepingRequest]>(
        async () => undefined,
      );
      ...
      await fireEvent.press(ui.getByTestId("housekeeping-submit"));
      await waitFor(() =>
        expect(submitRequest).toHaveBeenCalledWith(expectedInput),
      );
  ...
  it("trims notes without sending IDs when Stay and Reservation fixture identifiers change", async () => {
    ...
    const { ui } = await setup(
      ...
      {
        reservationId: "another-reservation-fixture",
        reservationStayId: "another-stay-fixture",
      },
    );
    await ready(ui);
    await fireEvent.changeText(
      ui.getByTestId("housekeeping-notes"),
      "  Primera línea\nSegunda línea  ",
    );
    await fireEvent.press(ui.getByTestId("housekeeping-submit"));
    await waitFor(() =>
    ...
    expect(submitRequest.mock.calls[0][0]).not.toHaveProperty(
  ...
  it("blocks rapid and pending double presses and shows success only after resolution", async () => {
    ...
    await fireEvent.press(button);
    await waitFor(() =>
      expect(ui.getByText("Enviando solicitud...")).toBeTruthy(),
    );
    await fireEvent.press(ui.getByTestId("housekeeping-submit"));
    expect(
      ui.getByTestId("housekeeping-submit").props.accessibilityState.disabled,
    ...
    expect(
      ui.getByTestId("housekeeping-type-selector").props.accessibilityState
        .disabled,
    ).toBe(true);
    ...
    await waitFor(() =>
      expect(ui.getByTestId("housekeeping-submit-success")).toBeTruthy(),
    );
    ...
    expect(
      ...
      expect.objectContaining({
        ...
        status: "REQUESTED",
        title: "Limpieza",
        summary: "11/09/2026 · 09:00–10:00",
      }),
    ]);
  });
  ...
  it.each([
    ...
    "preserves form on submit %s and manually retries the same payload",
    async (kind, error) => {
      const submitRequest = jest
        .fn<Promise<void>, [HousekeepingRequest]>()
      ...
      await fireEvent.changeText(
        ui.getByTestId("housekeeping-notes"),
        "  Mantener ventana cerrada  ",
      );
      await fireEvent.press(ui.getByTestId("housekeeping-submit"));
      await waitFor(() =>
        expect(ui.getByTestId(`housekeeping-submit-${kind}`)).toBeTruthy(),
      );
      expect(
        ui.queryByTestId(
          `housekeeping-submit-${kind === "error" ? "offline" : "error"}`,
        ),
      ).toBeNull();
      expect(ui.getByTestId("housekeeping-notes").props.value).toBe(
        "  Mantener ventana cerrada  ",
      );
      expect(ui.getByText("09:00–10:00")).toBeTruthy();
      expect(submitRequest).toHaveBeenCalledTimes(1);
      ...
      expect(submitRequest.mock.calls).toEqual([
        [{ ...expectedInput, notes: "Mantener ventana cerrada" }],
        [{ ...expectedInput, notes: "Mantener ventana cerrada" }],
      ]);
      expect(
        JSON.parse(
  ...
  it("shows Stay loading until the existing boundary resolves", async () => {
    ...
    const { ui } = await setup(undefined, {
      getCurrentStay: () => pending.promise,
    });
  ...
  it.each([
    ["error", new Error("stay failure")],
    ["offline", new NetworkError()],
  ] as const)(
    "distinguishes Stay %s and retries the existing query",
    async (kind, error) => {
      ...
      await waitFor(() =>
        expect(ui.getByTestId(`housekeeping-stay-${kind}`)).toBeTruthy(),
      );
      expect(ui.queryByTestId("housekeeping-submit")).toBeNull();
      expect(
        ui.queryByTestId(
          `housekeeping-stay-${kind === "error" ? "offline" : "error"}`,
        ),
      ).toBeNull();
  ...
  it("opens the productive route from Services, keeps Services active, and returns via Back and success", async () => {
    ...
    const ui = await renderRouter(
      {
        _layout: () => (
          <QueryClientProvider client={queryClient}>
            <ActiveReservationContextProvider
              initialActiveReservationContext={activeContext}
            >
              <SessionServiceRequestsProvider>
                <PathProbe />
                <Slot />
    ...
    await waitFor(() =>
      expect(ui.getByTestId("services-housekeeping-launcher")).toBeTruthy(),
    );
    ...
    await fireEvent.press(
      await ui.findByTestId("services-housekeeping-launcher"),
    );
    await ready(ui);
...
describe("Housekeeping shared selector chevrons", () => {
  it("renders the shared decorative chevron on cleaning type, date, and time selectors", async () => {
    const { ui } = await setup();
    await waitFor(() =>
      expect(ui.getByTestId("housekeeping-type-selector")).toBeTruthy(),
    );
    for (const testID of [
      "housekeeping-type-selector-chevron",
      "housekeeping-date-selector-chevron",
      "housekeeping-time-selector-chevron",
    ]) {
