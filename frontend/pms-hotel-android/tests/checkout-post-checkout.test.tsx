...
const emptyFolio = {
  checkoutTotal: { amountMinor: 0, currency: 'GTQ' as const, text: 'Total · Q0.00' },
  items: [],
  paidGuaranteeText: '',
  pendingBalanceText: '',
...
function CheckedOutProviders({ children }: PropsWithChildren) {
  const client = new QueryClient({
    defaultOptions: { mutations: { gcTime: 0, retry: false }, queries: { gcTime: 0, retry: false } },
  });
...
describe('Checkout service gates', () => {
  it.each([
    ['Limpieza', <HousekeepingScreen key="housekeeping" />, 'housekeeping-stay-completed', 'housekeeping-creation-blocked'],
    ['Amenidades', <AmenitiesScreen key="amenities" />, 'amenities-stay-completed', 'amenities-creation-blocked'],
    ['Room Service', <RoomServiceScreen key="room-service" />, 'room-service-stay-completed', 'room-service-creation-blocked'],
  ])('blocks direct %s creation after a checkout snapshot', async (_, screen, screenTestID, blockedTestID) => {
    const ui = await render(<CheckedOutProviders>{screen}</CheckedOutProviders>);
  ...
  it('blocks Late check-out after checkout while retaining Mis servicios', async () => {
    const ui = await render(<CheckedOutProviders><ServicesScreen service={new MockServicesService()} /></CheckedOutProviders>);
    await waitFor(() => expect(ui.getByTestId('services-stay-completed')).toBeTruthy());
