...
function LateCheckoutSeed({ checkoutUntil }: { checkoutUntil: string }) {
  ...
  useLayoutEffect(() => {
    ...
    seeded.current = true;
    addRequest({ kind: 'LATE_CHECKOUT', origin: 'SERVICES', status: 'REQUESTED', title: 'Late check-out', details: { type: 'LATE_CHECKOUT', serviceDate: '2026-09-18', checkoutUntil } });
...
async function setup(service: RoomServiceService = new MockRoomServiceService(), stayService: StayService = new MockStayService(), nowMs: () => number = () => afternoonNowMs, lateCheckoutUntil?: string, checkedOut = false) {
  const content = <>{lateCheckoutUntil ? <LateCheckoutSeed checkoutUntil={lateCheckoutUntil} /> : null}<RequestProbe /><RoomServiceScreen nowMs={nowMs} service={service} stayService={stayService} /></>;
  const guestContent = checkedOut ? <CheckoutSessionProvider><CheckedOutSeed />{content}</CheckoutSessionProvider> : content;
  const ui = await render(<QueryClientProvider client={client()}><SessionServiceRequestsProvider nowMs={nowMs}>{guestContent}</SessionServiceRequestsProvider></QueryClientProvider>);
  return { ui };
}
...
async function selectCategory(ui: Awaited<ReturnType<typeof render>>, period: 'BREAKFAST' | 'LUNCH' | 'DINNER' | 'BEVERAGES') {
  await fireEvent.press(ui.getByTestId('room-service-category-selector'));
  await waitFor(() => expect(ui.getByTestId('room-service-category-modal')).toBeTruthy());
  await fireEvent.press(ui.getByTestId(`room-service-category-option-${period}`));
}
...
async function chooseDeliveryTime(ui: Awaited<ReturnType<typeof render>>, hour = '13', minute = '45') {
  if (!ui.queryByTestId('room-service-schedule-panel')) {
    ...
    await waitFor(() => expect(ui.getByTestId('room-service-schedule-panel')).toBeTruthy());
  }
...
describe('Room Service — IMP-AND-0111', () => {
  it('replaces every mutable Room Service control with checkout due at effective checkout', async () => {
    ...
    const { ui } = await setup(new MockRoomServiceService({ submitRequest }), new MockStayService(), () => new Date(2026, 8, 18, 12, 0).getTime());
    await waitFor(() => expect(ui.getByTestId('room-service-checkout-due')).toBeTruthy());
  ...
  ] as const)('uses the typed Late checkout request at %s', async (_, nowMs, lifecycle) => {
    const { ui } = await setup(undefined, new MockStayService(), () => nowMs, '14:00');
    if (lifecycle === 'ACTIVE') {
      await ready(ui);
  ...
  it('keeps checked-out Room Service separate from checkout due', async () => {
    const { ui } = await setup(undefined, new MockStayService(), () => new Date(2026, 8, 18, 12, 0).getTime(), undefined, true);
    await waitFor(() => expect(ui.getByTestId('room-service-stay-completed')).toBeTruthy());
  ...
  it('keeps the cart content closed initially and opens an empty, closable panel from the accessible header button', async () => {
    ...
    expect(ui.getByRole('button', { name: 'Carrito' })).toBeTruthy(); expect(ui.getByTestId('room-service-cart-icon')).toBeTruthy();
    expect(ui.queryByTestId('room-service-cart-panel')).toBeNull();
    ...
    expect(ui.getByTestId('room-service-next').props.accessibilityState.disabled).toBe(true);
  ...
  it('renders categories and the approved mock catalog, including the nullable room context', async () => {
    const { ui } = await setup();
    await ready(ui);
    expect(ui.getByRole('button', { name: 'Seleccionar categoría' })).toBeTruthy();
    expect(ui.getByTestId('room-service-category-value').props.children).toBe('Desayunos');
    expect(ui.getByText('Desayuno continental')).toBeTruthy();
    expect(ui.getByText('Q 75')).toBeTruthy();
    await selectCategory(ui, 'LUNCH');
    expect(ui.getByTestId('room-service-category-value').props.children).toBe('Almuerzos');
  ...
  it('keeps cart lines and multiline notes after closing and reopening the cart', async () => {
    const { ui } = await setup();
    await ready(ui);
    await fireEvent.press(ui.getByTestId('room-service-product-continental-breakfast-increment'));
    await openCart(ui);
  ...
  it('filters only the selected category by trimmed, case- and accent-insensitive search', async () => {
    ...
    await fireEvent.changeText(ui.getByTestId('room-service-search'), '  TÍPICO ');
    expect(ui.getByText('Desayuno típico')).toBeTruthy();
    expect(ui.queryByText('Desayuno continental')).toBeNull();
    await fireEvent.changeText(ui.getByTestId('room-service-search'), 'café');
    expect(ui.getByText('No encontramos productos en esta categoría.')).toBeTruthy();
    await selectCategory(ui, 'BEVERAGES');
    expect(ui.getByText('Café')).toBeTruthy();
  });
  ...
  it('returns to the cart meal period after closing an incompatible category warning', async () => {
    ...
    expect(ui.getByText('Tu pedido ya contiene productos de Desayunos. Para agregar productos de Almuerzos, elimina primero esos productos del carrito.')).toBeTruthy();
    expect(ui.getByTestId('room-service-product-club-sandwich-increment').props.accessibilityState.disabled).toBe(true);
    await fireEvent.press(ui.getByTestId('room-service-product-club-sandwich-increment'));
    ...
    await fireEvent.press(ui.getByTestId('room-service-product-coffee-increment'));
    await openCart(ui);
    await fireEvent.press(ui.getByTestId('room-service-cart-item-continental-breakfast-decrement'));
    await fireEvent.press(ui.getByTestId('room-service-cart-close'));
    await selectCategory(ui, 'LUNCH');
    expect(ui.getByTestId('room-service-category-value').props.children).toBe('Almuerzos');
  });
  ...
  it('caps each Room Service product at five while allowing independent lines and their total badge', async () => {
    ...
    await ready(ui);
    await selectCategory(ui, 'BEVERAGES');
    for (let index = 0; index < 5; index += 1) await fireEvent.press(ui.getByTestId('room-service-product-coffee-increment'));
    expect(ui.getByTestId('room-service-product-coffee-increment').props.accessibilityState.disabled).toBe(true);
    await fireEvent.press(ui.getByTestId('room-service-product-coffee-increment'));
    expect(ui.getByLabelText('Cantidad de Café: 5')).toBeTruthy();
    await fireEvent.press(ui.getByTestId('room-service-product-natural-juice-increment'));
    await fireEvent.press(ui.getByTestId('room-service-product-natural-juice-increment'));
    await fireEvent.press(ui.getByTestId('room-service-product-natural-juice-increment'));
    await fireEvent.press(ui.getByTestId('room-service-product-natural-juice-increment'));
    await fireEvent.press(ui.getByTestId('room-service-product-natural-juice-increment'));
    ...
    expect(ui.getByTestId('room-service-cart-item-coffee-increment').props.accessibilityState.disabled).toBe(true);
    await fireEvent.press(ui.getByTestId('room-service-cart-close'));
  ...
  it('keeps Lunch through 13:30, but not after, at 10:59 with Late checkout', async () => {
    const nowMs = () => new Date(2026, 8, 18, 10, 59).getTime();
    const { ui } = await setup(undefined, new MockStayService({ kind: 'success', dto: { ...currentStayFixture, departure: '2026-09-18' } }), nowMs, '14:00');
    await ready(ui);
    await selectCategory(ui, 'LUNCH');
    await fireEvent.press(ui.getByTestId('room-service-product-club-sandwich-increment'));
    await openCart(ui);
    ...
    await fireEvent.press(ui.getByTestId('room-service-delivery-time-picker-hour-13'));
    await fireEvent.press(ui.getByTestId('room-service-delivery-time-picker-minute-30'));
    await waitFor(() => expect(ui.getByTestId('room-service-delivery-time-picker-confirm').props.accessibilityState.disabled).toBe(false));
    await fireEvent.press(ui.getByTestId('room-service-delivery-time-picker-minute-31'));
    await waitFor(() => expect(ui.getByTestId('room-service-delivery-time-picker-confirm').props.accessibilityState.disabled).toBe(true));
  });
  ...
  it.each(['', '   \n  '])('omits whitespace-only notes from the request (%j)', async (notes) => {
    ...
    await chooseDeliveryTime(ui);
    await fireEvent.press(ui.getByTestId('room-service-submit'));
    await waitFor(() => expect(submitRequest).toHaveBeenCalledWith({ items: [{ itemFixtureKey: 'club-sandwich', quantity: 1, mealPeriod: 'LUNCH' }], deliveryTime: '13:45', serviceDate: '2026-09-11' }));
  });
  ...
  it('trims multiline notes and sends no Stay, Reservation, Room, total, or price data', async () => {
    ...
    await waitFor(() => expect(submitRequest).toHaveBeenCalledWith({
      items: [{ itemFixtureKey: 'club-sandwich', quantity: 1, mealPeriod: 'LUNCH' }], deliveryTime: '13:45', serviceDate: '2026-09-11', notes: 'Sin cebolla\nPor favor',
    }));
  ...
  it('shows submitting, blocks duplicate confirmation, and renders success only after resolution', async () => {
    ...
    expect(JSON.parse(ui.getByTestId('session-service-requests-probe').props.children)).toEqual([
      expect.objectContaining({ kind: 'ROOM_SERVICE', origin: 'SERVICES', status: 'REQUESTED', title: 'Room Service', summary: '11/09/2026 · 13:45 · 1 producto' }),
    ]);
  });
  ...
  it.each([
    ...
    expect(ui.getByTestId('room-service-cart-line-club-sandwich')).toBeTruthy();
  ...
  it('uses the shared drawer navigation guard for dirty carts without resetting on the current route', async () => {
    ...
    await fireEvent.press(ui.getByTestId('guest-child-header-menu'));
    await waitFor(() => expect(ui.getByTestId('guest-navigation-drawer-link-room-service')).toBeTruthy());
    await fireEvent.press(ui.getByTestId('guest-navigation-drawer-link-room-service'));
    expect(ui.queryByTestId('guest-navigation-discard-modal')).toBeNull();
    expect(ui.getByTestId('room-service-cart-badge').props.children).toBe(1);

    await fireEvent.press(ui.getByTestId('guest-child-header-menu'));
    await fireEvent.press(ui.getByTestId('guest-navigation-drawer-section-stay'));
    await fireEvent.press(ui.getByTestId('guest-navigation-drawer-link-inicio'));
    await waitFor(() => expect(ui.getByTestId('guest-navigation-discard-modal')).toBeTruthy());
    expect(ui.getByTestId('pathname').props.children).toBe('/services/room-service');
    await fireEvent.press(ui.getByTestId('guest-navigation-discard-modal-cancel'));
    expect(ui.queryByTestId('guest-navigation-discard-modal')).toBeNull();
    expect(ui.getByTestId('room-service-cart-badge').props.children).toBe(1);
    ...
    await fireEvent.press(ui.getByTestId('guest-navigation-discard-modal-confirm'));
  ...
  it('closes the category selector only from its backdrop and preserves the active draft', async () => {
    ...
    await ready(ui);
    await fireEvent.press(ui.getByTestId('room-service-product-continental-breakfast-increment'));
    await fireEvent.changeText(ui.getByTestId('room-service-search'), 'continental');
    await fireEvent.press(ui.getByTestId('room-service-category-selector'));
    await waitFor(() => expect(ui.getByTestId('room-service-category-modal')).toBeTruthy());
    ...
    expect(ui.getByTestId('room-service-product-continental-breakfast')).toBeTruthy();
    expect(ui.getByTestId('room-service-category-value').props.children).toBe('Desayunos');
    expect(ui.getByTestId('room-service-search').props.value).toBe('continental');
    expect(ui.getByTestId('room-service-cart-badge').props.children).toBe(1);
  });
