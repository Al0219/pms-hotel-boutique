...
const primaryReservationContext = {
...
function createControlledService({
  mode = "success",
}: { mode?: ControlledServiceMode } = {}): ControlledCheckoutService {
  let callCount = 0;
  ...
  return {
    service: {
      ...
      generateInvoicePdf: async () => ({
        fileDisplayText: "PDF simulado generado",
      }),
...
function SeedTransfer() {
  ...
  useEffect(() => {
    addRequest(
      buildTransferSessionRequestInput({
        destinationKey: "airport",
        estimatedAmount: 120,
        fareText: "Q 120",
        passengers: 2,
        scheduledAtMs: new Date(2099, 7, 30, 10).getTime(),
        summary: "Traslado",
      }),
    );
  }, [addRequest]);
...
const SessionRequestsTestController = forwardRef<SessionRequestsTestControl>(
  function SessionRequestsTestController(_, ref) {
    const { removeRequest, requests, updateRequest } =
      useSessionServiceRequests();
    const roomService = requests.find(
      (request) => request.kind === "ROOM_SERVICE",
    );
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        removeRoomService: () =>
          roomService ? removeRequest(roomService.sessionRequestId) : false,
        removeTransfer: () =>
          transfer ? removeRequest(transfer.sessionRequestId) : false,
        updateRoomService: () =>
          roomService
            ? updateRequest(
                roomService.sessionRequestId,
                roomServiceRequestInput("Café actualizado"),
              )
            : false,
      }),
...
function createGuestLayout(
  sessionControl?: RefObject<SessionRequestsTestControl | null>,
) {
      ...
      <QueryClientProvider client={queryClient}>
        <SessionServiceRequestsProvider>
          <CheckoutSessionProvider>
            ...
            >
              <ActiveReservationContextProvider
                initialActiveReservationContext={primaryReservationContext}
              >
                <GuestNavigationMenuProvider>
                  {sessionControl ? (
                    <SessionRequestsTestController ref={sessionControl} />
                  ) : null}
                  <Slot />
                </GuestNavigationMenuProvider>
              </ActiveReservationContextProvider>
...
async function finishCheckout(ui: Awaited<ReturnType<typeof renderRouter>>) {
  ...
  await waitFor(() =>
    expect(ui.getByTestId("checkout-confirmation")).toBeTruthy(),
  );
  await act(async () => {
...
describe("Checkout / Invoice routes", () => {
  it("resolves Account through the Guest route tree", async () => {
    ...
    await waitFor(() =>
      expect(ui.getByTestId("account-stay-hub-screen")).toBeTruthy(),
    );
  });
  ...
  it("resolves Checkout through the Guest route tree", async () => {
    ...
    await act(async () => {
      fireEvent.press(ui.getByTestId("guest-child-header-menu"));
    });
    await waitFor(() =>
      expect(ui.getByTestId("guest-navigation-drawer-panel")).toBeTruthy(),
    );
    await act(async () => {
      fireEvent.press(ui.getByTestId("guest-navigation-drawer-close"));
    });
    await act(async () => {
      fireEvent.press(ui.getByTestId("checkout-back"));
    });
    await waitFor(() =>
      expect(ui.getByTestId("account-stay-hub-screen")).toBeTruthy(),
    );
  ...
  it("returns direct Invoice without a snapshot to Account", async () => {
    ...
    await waitFor(() =>
      expect(ui.getByTestId("invoice-no-snapshot")).toBeTruthy(),
    );
    expect(ui.getByTestId("guest-child-header-menu")).toBeTruthy();
    await act(async () => {
      fireEvent.press(ui.getByTestId("guest-child-header-menu"));
...
describe("Checkout mutation integration", () => {
  it("blocks checkout before the local departure date without a mutation", async () => {
    ...
    const ControlledCheckoutRoute = createControlledCheckoutRoute(
      controlled,
      () => new Date("2026-09-17T23:59:00"),
    );
    const ui = await renderRouter(
      {
        _layout: GuestLayout,
    ...
    await waitFor(() =>
      expect(ui.getByTestId("checkout-before-departure")).toBeTruthy(),
    );
    expect(
      ui.getByTestId("checkout-submit").props.accessibilityState.disabled,
    ).toBe(true);
    await act(async () => {
      fireEvent.press(ui.getByTestId("checkout-submit"));
    });
  ...
  it("requires final confirmation on and after departure", async () => {
    ...
    expect(controlled.callCount).toBe(0);
    await act(async () => {
      fireEvent.press(ui.getByTestId("checkout-confirmation-cancel"));
    });
    expect(controlled.callCount).toBe(0);
  ...
  it("keeps the frozen Invoice stable after live session update and delete", async () => {
    ...
    await waitFor(() =>
      expect(sessionControl.current?.transferRequestId).toBeTruthy(),
    );
    await waitFor(() => expect(ui.getByTestId("checkout-submit")).toBeTruthy());
    ...
    const referenceText = String(
      ui.getByTestId("invoice-reference").props.children,
    );
    const generatedAtText = String(
      ui.getByTestId("invoice-generated-at").props.children,
    );
    ...
    await waitFor(() => expect(sessionControl.current?.requestId).toBeTruthy());
    expect(ui.getByTestId("invoice-reference").props.children).toBe(
      referenceText,
    );
    expect(ui.getByTestId("invoice-generated-at").props.children).toBe(
      generatedAtText,
    );
    expect(ui.getByText("Café × 1")).toBeTruthy();
    ...
    await waitFor(() =>
      expect(sessionControl.current?.requestId).toBeUndefined(),
    );
    expect(ui.getByTestId("invoice-reference").props.children).toBe(
      referenceText,
    );
  ...
  it("returns to Account rather than completed Checkout after success", async () => {
    ...
    await waitFor(() =>
      expect(ui.getByTestId("account-checkout-launcher")).toBeTruthy(),
    );
  ...
  it("stays on Checkout when checkout service returns a generic error", async () => {
    ...
    await waitFor(() =>
      expect(ui.getByText("No pudimos confirmar Check-out")).toBeTruthy(),
    );
    expect(controlled.callCount).toBe(1);
    expect(controlled.lastInput).toEqual({});
    expect(ui.getByTestId("checkout-screen")).toBeTruthy();
