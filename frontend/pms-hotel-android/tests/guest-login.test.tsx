...
function MutationCacheProbe({ service }: { service: GuestAuthService }) {
  ...
  return (
    <Pressable
      testID="guest-login-mutation-cache-probe"
      onPress={() =>
        login.submit({
...
function SessionProbe() {
  ...
  return (
    <View>
      ...
      <Text testID="guest-login-context">
        {activeReservationContext
          ? `${activeReservationContext.reservationId}/${activeReservationContext.reservationStayId}`
          : "none"}
      </Text>
    </View>
  );
...
async function renderLogin(
  ...
  return render(
    <LoginProviders initialContext={options.initialContext}>
      <LoginScreen
...
async function fillValidLogin(screen: Awaited<ReturnType<typeof renderLogin>>) {
  await fireEvent.changeText(
    screen.getByTestId("guest-login-email"),
    "guest@example.com",
  );
  await fireEvent.changeText(
    screen.getByTestId("guest-login-password"),
    "guest-demo-password",
  );
}
...
describe("Guest Login", () => {
  it("renders the approved main entry without account creation, recovery, or social actions", async () => {
    ...
    expect(
      screen.queryByText(
        /Crear cuenta|Registrarse|Olvidé|Google|Apple|Facebook/i,
      ),
    ).toBeNull();
  ...
  it("redirects root to /login", async () => {
    function LoginRoute() {
      return (
        <LoginProviders>
          <LoginScreen service={{ login: jest.fn() }} />
        </LoginProviders>
      );
    }
    const screen = await renderRouter(
      { index: IndexRoute, login: LoginRoute },
      { initialUrl: "/" },
    );
    await waitFor(() =>
      expect(screen.getByTestId("guest-login-screen")).toBeTruthy(),
  ...
  it("does not retain password as a TanStack Mutation variable", async () => {
    ...
    const service: GuestAuthService = {
      login: jest
        .fn()
        .mockResolvedValue({ accountId: "guest-account-primary" }),
    };
    const screen = await render(
      <QueryClientProvider client={client}>
        <MutationCacheProbe service={service} />
      </QueryClientProvider>,
    );
    await fireEvent.press(
      screen.getByTestId("guest-login-mutation-cache-probe"),
    );
    await waitFor(() => expect(service.login).toHaveBeenCalledTimes(1));
    expect(
      client.getMutationCache().getAll()[0]?.state.variables,
    ).toBeUndefined();
  });
  ...
  it("uses a neutral empty-email placeholder and an internal accessible password-eye toggle", async () => {
    ...
    expect(
      screen.getByTestId("guest-login-password-toggle-icon").props.children
        .props.name,
    ).toEqual({
    ...
    expect(
      screen.getByTestId("guest-login-password-toggle-icon").props.children
        .props.name,
    ).toEqual({ android: "visibility", ios: "eye", web: "visibility" });
    await fireEvent.press(toggle);
    expect(password.props.secureTextEntry).toBe(true);
  });
  ...
  it("trims email, submits once, starts the session, clears prior context, and invokes the injected handoff", async () => {
    ...
    const login = jest
      .fn<Promise<GuestAuthSession>, [unknown]>()
    ...
    expect(screen.getByTestId("guest-login-session").props.children).toBe(
      "guest-account-authenticated",
    );
    expect(screen.getByTestId("guest-login-context").props.children).toBe(
      "none",
    );
  });
  ...
  it("blocks duplicated submission and disables mutable controls while submitting", async () => {
    ...
    const pending = new Promise<GuestAuthSession>((resolve) => {
    ...
    expect(screen.getByTestId("guest-login-password").props.editable).toBe(
      false,
    );
    expect(
      screen.getByTestId("guest-login-password-toggle").props.accessibilityState
        .disabled,
    ).toBe(true);
  ...
  it("uses generic, non-enumerating copy for invalid credentials and clears stale state when input changes", async () => {
    const screen = await renderLogin({
      login: jest.fn().mockRejectedValue(new InvalidGuestCredentialsError()),
    });
    await fillValidLogin(screen);
    await fireEvent.press(screen.getByTestId("guest-login-submit"));
    await waitFor(() =>
      expect(
        screen.getByTestId("guest-login-invalid-credentials"),
      ).toBeTruthy(),
    );
    expect(screen.getByText("No pudimos iniciar sesión")).toBeTruthy();
    expect(
      screen.getByText("Revisa tus datos e inténtalo de nuevo."),
    ).toBeTruthy();
    expect(
      screen.queryByText(/correo inexistente|contraseña incorrecta/i),
    ).toBeNull();
    await fireEvent.changeText(
      screen.getByTestId("guest-login-email"),
    ...
    await waitFor(() =>
      expect(
        screen.queryByTestId("guest-login-invalid-credentials"),
      ).toBeNull(),
    );
    expect(screen.getByTestId("guest-login-password").props.value).toBe(
      "guest-demo-password",
    );
  });
  ...
  it.each([
    ...
    "renders %s state and retries without clearing fields",
    async (_name, failure, testID, title, body) => {
      const login = jest
        .fn<Promise<GuestAuthSession>, [unknown]>()
        .mockRejectedValueOnce(failure)
        .mockResolvedValueOnce({ accountId: "guest-account-primary" });
      const onAuthenticated = jest.fn();
      ...
      expect(screen.getByTestId("guest-login-email").props.value).toBe(
        "guest@example.com",
      );
      expect(screen.getByTestId("guest-login-password").props.value).toBe(
        "guest-demo-password",
  ...
  it("hands off successful login to /reservations for linked-stay resolution", async () => {
    ...
    await fireEvent.press(screen.getByTestId("guest-login-submit"));
    await waitFor(() =>
      expect(replaceSpy).toHaveBeenCalledWith("/reservations"),
    );
