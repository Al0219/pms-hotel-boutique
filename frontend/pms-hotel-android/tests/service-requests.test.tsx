...
const sampleRequest: AddSessionServiceRequestInput = {
  ...
  origin: "SERVICES",
  status: "REQUESTED",
  summary: "Limpieza completa · 10:00–11:00",
  title: "Limpieza",
};
...
function RemoveRequestButton({
  ...
  return (
    <Pressable
      onPress={() => removeRequest(sessionRequestId)}
      testID={`remove-${sessionRequestId}`}
    />
  );
}
...
function SetCheckoutDueClock() {
  ...
  return (
    <Pressable
      onPress={() => setCustomDate(new Date(checkoutDueNowMs))}
      testID="set-checkout-due-clock"
    />
  );
}
async function renderRequestsScreen({
  requests = [] as readonly AddSessionServiceRequestInput[],
  nowMs = () => activeNowMs,
  lateCheckoutUntil,
  checkedOut = false,
}: {
  ...
  const content = (
    <>
      ...
      <SeedRequests requests={requests} />
      <SessionServiceRequestsScreen nowMs={nowMs} />
    </>
  );
...
describe("Session service requests — IMP-AND-0112", () => {
  it("reacts to an AppClock transition without remounting and closes mutable controls", async () => {
    const request: AddSessionServiceRequestInput = {
      kind: "HOTEL_ASSIGNED",
      origin: "CHAT",
      status: "ASSIGNED",
      title: "Servicio reactivo",
      details: { type: "HOTEL_ASSIGNED" },
    };
    const ui = await render(
      <AppClockProvider
        initialCustomDate={new Date(2026, 8, 18, 11, 59)}
        initialMode="CUSTOM"
      >
        <QueryClientProvider
          client={
    ...
    await waitFor(() =>
      expect(ui.getByLabelText("Editar Servicio reactivo")).toBeTruthy(),
    );
    await fireEvent.press(ui.getByTestId("set-checkout-due-clock"));
    await waitFor(() =>
      expect(ui.queryByLabelText("Editar Servicio reactivo")).toBeNull(),
    );
  ...
  it("keeps cards, filters, and completed details available while checkout is due", async () => {
    const completed: AddSessionServiceRequestInput = {
      kind: "ROOM_SERVICE",
      origin: "SERVICES",
      status: "COMPLETED",
      title: "Pedido completado",
      summary: "Servicio entregado",
      details: {
        type: "ROOM_SERVICE",
        serviceDate: "2026-09-18",
        deliveryTime: "10:00",
    ...
    const active: AddSessionServiceRequestInput = {
      kind: "HOTEL_ASSIGNED",
      origin: "CHAT",
      status: "ASSIGNED",
    ...
    const ui = await renderRequestsScreen({
      nowMs: () => checkoutDueNowMs,
      requests: [completed, active],
    });
    ...
    await waitFor(() =>
      expect(ui.queryByLabelText("Editar Servicio activo")).toBeNull(),
    );
    expect(
      ui.queryByTestId("complete-session-service-request-session-request-1"),
    ).toBeNull();
    expect(
      ui.queryByTestId(
        "remove-session-service-request-cutoff-session-request-1",
      ),
    ).toBeNull();
    await fireEvent.press(
      ui.getByTestId("session-service-request-filter-COMPLETED"),
    );
    await fireEvent.press(
      ui.getByTestId("session-service-request-session-request-1"),
    );
    expect(
      ui.getByTestId("session-service-request-details-session-request-1"),
    ).toBeTruthy();
    ui.unmount();
  });
  ...
  it("keeps an expired Late checkout visible but non-cancelable in checkout due", async () => {
    const ui = await renderRequestsScreen({
      nowMs: () => new Date(2026, 8, 18, 14, 0).getTime(),
  ...
  it("keeps completed history and details read-only after a checkout snapshot", async () => {
    ...
    await waitFor(() =>
      expect(ui.getByText("Servicio no modificable")).toBeTruthy(),
    );
    await waitFor(() =>
      expect(ui.queryByLabelText("Editar Servicio no modificable")).toBeNull(),
    );
    await fireEvent.press(
      ui.getByTestId("session-service-request-filter-COMPLETED"),
    );
  ...
  it("starts empty, keeps newest first, supports types/origins, and dedupes local events", () => {
    ...
    const newer = {
      kind: "HOTEL_ASSIGNED" as const,
      origin: "CHAT" as const,
      status: "ASSIGNED" as const,
      title: "Traslado al aeropuerto",
      createdAtMs: 20,
      sessionRequestId: "session-request-2",
    };
    const once = sessionServiceRequestsReducer(
      initialSessionServiceRequestsState,
      { type: "ADD_REQUEST", request: older, dedupeKey: "one" },
    );
    const ordered = sessionServiceRequestsReducer(once, {
      type: "ADD_REQUEST",
      request: newer,
      dedupeKey: "two",
    });
    const duplicated = sessionServiceRequestsReducer(ordered, {
      type: "ADD_REQUEST",
      request: newer,
      dedupeKey: "two",
    });
    ...
    expect(ordered.requests.map((request) => request.sessionRequestId)).toEqual(
      ["session-request-2", "session-request-1"],
    );
    expect(ordered.requests[0]).toMatchObject({
      kind: "HOTEL_ASSIGNED",
      origin: "CHAT",
      status: "ASSIGNED",
    });
  ...
  it("removes only the selected session request and preserves the remaining newest-first order", () => {
    const first = sessionServiceRequestsReducer(
      initialSessionServiceRequestsState,
      {
        type: "ADD_REQUEST",
        request: {
    ...
    const second = sessionServiceRequestsReducer(first, {
      type: "ADD_REQUEST",
      request: {
        ...sampleRequest,
        createdAtMs: 30,
        sessionRequestId: "session-request-3",
      },
    });
    const third = sessionServiceRequestsReducer(second, {
      type: "ADD_REQUEST",
      request: {
        ...sampleRequest,
        createdAtMs: 20,
    ...
    const removed = sessionServiceRequestsReducer(third, {
      type: "REMOVE_REQUEST",
      sessionRequestId: "session-request-2",
    });

    expect(removed.requests.map((request) => request.sessionRequestId)).toEqual(
      ["session-request-3", "session-request-1"],
    );
  });
  ...
  it("uses the complete editable card for edit without a visible Edit button", async () => {
    ...
    const request = {
      ...
      sessionRequestId: "session-request-9",
      createdAtMs: 9,
    };
    const rendered = await render(
      <SessionServiceRequestCard onEdit={onEdit} request={request} />,
    );
  ...
  it("maps every request kind to its editor route", () => {
    const request = {
      ...sampleRequest,
      sessionRequestId: "session-request-10",
      createdAtMs: 10,
    };
    expect(
      sessionServiceRequestEditPath({ ...request, kind: "ROOM_SERVICE" }),
    ).toBe("/services/room-service");
    expect(
      sessionServiceRequestEditPath({ ...request, kind: "VEHICLE_REQUEST" }),
    ).toBe("/valet");
    expect(
      sessionServiceRequestEditPath({ ...request, kind: "TRANSFER" }),
    ).toBe("/valet");
    expect(
      sessionServiceRequestEditPath({ ...request, kind: "HOUSEKEEPING" }),
    ).toBe("/services/housekeeping");
    expect(
      sessionServiceRequestEditPath({ ...request, kind: "LATE_CHECKOUT" }),
    ).toBe("/services/requests");
    expect(
      sessionServiceRequestEditPath({ ...request, kind: "HOTEL_ASSIGNED" }),
    ).toBe("/services/requests");
  });
  ...
  it("filters the session-only history by every approved status without changing its order", () => {
    ...
    expect(
      filterServiceRequests(requests, "ACTIVE").map(
        (item) => item.sessionRequestId,
      ),
    ).toEqual(["b", "a"]);
    expect(
      filterServiceRequests(requests, "REQUESTED").map(
        (item) => item.sessionRequestId,
      ),
    ).toEqual(["a"]);
    expect(
      filterServiceRequests(requests, "ASSIGNED").map(
        (item) => item.sessionRequestId,
      ),
    ).toEqual(["b"]);
    expect(
      filterServiceRequests(requests, "COMPLETED").map(
        (item) => item.sessionRequestId,
      ),
    ).toEqual(["c"]);
    expect(
      filterServiceRequests(requests, "ALL").map(
        (item) => item.sessionRequestId,
      ),
    ).toEqual(["c", "b", "a"]);
  });
  ...
  it("uses an injected current time for the exact thirty-minute creation boundary and time-only midnight limit", () => {
    ...
    expect(
      hasServiceTimeAvailableToday(new Date(2026, 0, 1, 23, 29).getTime()),
    ).toBe(true);
    expect(
      hasServiceTimeAvailableToday(new Date(2026, 0, 1, 23, 30).getTime()),
    ).toBe(false);
    expect(
      isServiceTimeTextAllowed("00:30", new Date(2026, 0, 1, 23, 45).getTime()),
    ).toBe(false);
  });
  ...
  it("combines local YYYY-MM-DD dates with time or slot starts for creation and future-date cutoff", () => {
    ...
    expect(getServiceDateTimeMs("2026-09-16", "08:00–09:00")).toBe(
      new Date(2026, 8, 16, 8, 0).getTime(),
    );
    expect(
      getNearestServiceTime("2026-09-15", ["13:36", "13:37", "13:38"], now),
    ).toBe("13:37");
    expect(
      getNearestServiceTime("2026-09-16", ["08:00", "13:37"], now, "13:37"),
    ).toBe("13:37");
  });
  ...
  it("keeps every guest-selected service date inside the inclusive reservation boundary", () => {
    ...
    expect(
      isServiceDateWithinStay("2026-09-15", now, arrival, "2026-09-18"),
    ).toBe(true);
    expect(
      isServiceDateWithinStay("2026-09-18", now, arrival, "2026-09-18"),
    ).toBe(true);
    expect(
      isServiceDateWithinStay("2026-09-19", now, arrival, "2026-09-18"),
    ).toBe(false);
    expect(
      getFirstAvailableServiceDate(
        new Date(2026, 8, 18, 14, 31).getTime(),
    ...
    expect(
      getFirstAvailableServiceDate(
        new Date(2026, 8, 18, 23, 31).getTime(),
        ["00:00", "13:37"],
        arrival,
        "2026-09-18",
      ),
  ...
  it("derives the inclusive local scheduling window from arrival, today, and departure", () => {
    ...
    expect(
      getStayServiceDateWindow(
        arrival,
        departure,
        new Date(2026, 8, 18, 9).getTime(),
      ),
    ).toEqual({ minimumDate: arrival, maximumDate: departure });
    expect(
      getStayServiceDateWindow(
        arrival,
        departure,
        new Date(2026, 8, 22, 9).getTime(),
      ),
    ).toEqual({ minimumDate: "2026-09-22", maximumDate: departure });
    expect(
      getStayServiceDateWindow(
        arrival,
        departure,
    ...
    ).toEqual({ minimumDate: departure, maximumDate: departure });
    expect(
      getStayServiceDateWindow(
        arrival,
        departure,
  ...
  it("uses the approved frontend/mock normal checkout policy for point services and slot ends", () => {
    ...
    expect(getStayCheckoutDateTimeMs(departure)).toBe(
  ...
  it("never makes late checkout completable", () => {
    const late = {
      ...sampleRequest,
      kind: "LATE_CHECKOUT" as const,
      sessionRequestId: "late-checkout",
      createdAtMs: 0,
      details: {
    ...
    expect(
      canCompleteServiceRequest(late, new Date(2026, 8, 18, 12, 0).getTime()),
    ).toBe(false);
    expect(
      canCompleteServiceRequest(late, new Date(2026, 8, 18, 14, 0).getTime()),
    ).toBe(false);
  });

  it("enables completion only at the configured service moment, using slot end for housekeeping", () => {
    const make = (
      kind: SessionServiceRequest["kind"],
      details: NonNullable<SessionServiceRequest["details"]>,
    ): SessionServiceRequest => ({
      sessionRequestId: kind,
      kind,
      origin: "SERVICES",
      status: "REQUESTED",
      title: kind,
    ...
    const room = make("ROOM_SERVICE", {
      type: "ROOM_SERVICE",
      serviceDate: "2026-09-16",
      deliveryTime: "14:30",
      items: [],
    });
    const vehicle = make("VEHICLE_REQUEST", {
      type: "VEHICLE_REQUEST",
      serviceDate: "2026-09-16",
      requestedTime: "14:30",
      sessionVehicleId: "vehicle",
    });
    const transfer = make("TRANSFER", {
      type: "TRANSFER",
      destinationKey: "airport",
      scheduledAtMs: new Date(2026, 8, 16, 14, 30).getTime(),
      passengers: 1,
    });
    const housekeeping = make("HOUSEKEEPING", {
      type: "HOUSEKEEPING",
      serviceDate: "2026-09-16",
      timeSlot: "11:00–12:00",
      cleaningType: "FULL_CLEANING",
    });
    const late = make("LATE_CHECKOUT", {
      type: "LATE_CHECKOUT",
      serviceDate: "2026-09-16",
      checkoutUntil: "14:00",
    });
    ...
    expect(
      canCompleteServiceRequest(room, new Date(2026, 8, 16, 14, 29).getTime()),
    ).toBe(false);
    expect(
      canCompleteServiceRequest(room, new Date(2026, 8, 16, 14, 30).getTime()),
    ).toBe(true);
    expect(
      canCompleteServiceRequest(
        vehicle,
        new Date(2026, 8, 16, 14, 29).getTime(),
      ),
    ).toBe(false);
    expect(
      canCompleteServiceRequest(
        vehicle,
        new Date(2026, 8, 16, 14, 30).getTime(),
      ),
    ).toBe(true);
    expect(
      canCompleteServiceRequest(
    ...
    expect(
      canCompleteServiceRequest(
        housekeeping,
        new Date(2026, 8, 16, 11, 59).getTime(),
      ),
    ).toBe(false);
    expect(getServiceRequestCompletionEligibleAt(housekeeping)).toBe(
      new Date(2026, 8, 16, 12, 0).getTime(),
    );
    expect(
      canCompleteServiceRequest(
        housekeeping,
    ...
    expect(
      canCompleteServiceRequest(late, new Date(2026, 8, 16, 13, 59).getTime()),
    ).toBe(false);
    expect(
      canCompleteServiceRequest(late, new Date(2026, 8, 16, 14, 0).getTime()),
    ).toBe(false);
  ...
  it("keeps the check in a separate right-side action and never opens edit from it", async () => {
    ...
    const request = {
      ...sampleRequest,
      sessionRequestId: "separate-check",
      createdAtMs: 1,
      details: {
    ...
    const rendered = await render(
      <SessionServiceRequestCard
        nowMs={new Date(2026, 0, 1, 14, 0).getTime()}
    ...
    await fireEvent.press(
      rendered.getByTestId("complete-session-service-request-separate-check"),
    );
    expect(onEdit).not.toHaveBeenCalled();
    expect(
      rendered.getByTestId(
        "complete-session-service-request-modal-separate-check",
      ),
    ).toBeTruthy();
    await fireEvent.press(
      rendered.getByTestId(
        "complete-session-service-request-modal-separate-check-confirm",
      ),
    );
  ...
  it("keeps configurable request kinds tappable and leaves fixed Late check-out as detail-only", async () => {
        ...
        {requests.map((request, index) => (
          <SessionServiceRequestCard
            ...
            request={{
              ...request,
              createdAtMs: index,
              sessionRequestId: `session-request-edit-${index}`,
            }}
          />
        ))}
      </View>,
    ...
    for (const request of requests.filter(
      ...
      await fireEvent.press(rendered.getByLabelText(`Editar ${request.title}`));
  ...
  it("marks requests completed without deleting them from session state", () => {
    const request = {
      ...sampleRequest,
      createdAtMs: 10,
      sessionRequestId: "session-request-completed",
    };
    const seeded = sessionServiceRequestsReducer(
      initialSessionServiceRequestsState,
      { type: "ADD_REQUEST", request },
    );
    const completed = sessionServiceRequestsReducer(seeded, {
  ...
  it("keeps vehicle requests for distinct local dates and times separate, completing only the exact request", () => {
    const first = {
      kind: "VEHICLE_REQUEST" as const,
      origin: "VALET" as const,
      status: "REQUESTED" as const,
      title: "Solicitud de vehículo",
      createdAtMs: 1,
      sessionRequestId: "vehicle-request-1",
      details: {
        type: "VEHICLE_REQUEST" as const,
        sessionVehicleId: "vehicle-1",
        serviceDate: "2026-09-16",
        requestedTime: "08:00",
      },
    };
    const second = {
      ...first,
      createdAtMs: 2,
      sessionRequestId: "vehicle-request-2",
      details: {
        ...first.details,
        serviceDate: "2026-09-17",
        requestedTime: "09:30",
    ...
    const state = sessionServiceRequestsReducer(
      sessionServiceRequestsReducer(initialSessionServiceRequestsState, {
        type: "ADD_REQUEST",
        request: first,
      }),
      { type: "ADD_REQUEST", request: second },
    );
    const completed = sessionServiceRequestsReducer(state, {
      type: "COMPLETE_REQUEST",
      sessionRequestId: second.sessionRequestId,
    });

    expect(completed.requests).toHaveLength(2);
    expect(
      completed.requests.find(
        (request) => request.sessionRequestId === first.sessionRequestId,
      ),
    ).toMatchObject({ status: "REQUESTED", details: first.details });
    expect(
      completed.requests.find(
        (request) => request.sessionRequestId === second.sessionRequestId,
      ),
    ).toMatchObject({ status: "COMPLETED", details: second.details });
  });
  ...
  it("hides completed requests from the Account preview and canonical active list", async () => {
    ...
    const requests = [
      completed,
      { ...sampleRequest, title: "Servicio activo" },
    ];
    ...
    const account = await render(
      ...
      >
        ...
        >
          <SessionServiceRequestsProvider>
            <SeedRequests requests={requests} />
            <AccountStayHubScreen service={new MockStayService()} />
          </SessionServiceRequestsProvider>
        </ActiveReservationContextProvider>
      </QueryClientProvider>,
    );
    await waitFor(() =>
      expect(account.getByText("Servicio activo")).toBeTruthy(),
    );
  ...
  it("keeps Account stay content, renders an empty preview, and limits the preview to three requests", async () => {
    const requests: AddSessionServiceRequestInput[] = [
      sampleRequest,
      {
        ...
        status: "REQUESTED",
        title: "Room Service",
        summary: "2 productos · Entrega 13:45",
      },
      {
    ...
    expect(
      empty.getByText("Aún no tienes servicios solicitados."),
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(populated.getByText("Room Service")).toBeTruthy(),
    );
    ...
    const rendered = await render(
      ...
      >
        ...
        >
          <SessionServiceRequestsProvider>
            <SeedRequests requests={requests} />
            <RemoveRequestButton sessionRequestId="session-request-3" />
            <AccountStayHubScreen service={new MockStayService()} />
          </SessionServiceRequestsProvider>
        </ActiveReservationContextProvider>
      </QueryClientProvider>,
  ...
  it("navigates from Account preview to the canonical route, keeps Servicios active, and returns to Services", async () => {
              ...
              <GuestAuthSessionProvider>
                ...
                >
                  <GuestNavigationMenuProvider>
                    <SessionServiceRequestsProvider>
                      <SeedRequests requests={[sampleRequest]} />
                      <PathProbe />
                      <Slot />
                    </SessionServiceRequestsProvider>
                  </GuestNavigationMenuProvider>
    ...
    await waitFor(() =>
      expect(ui.getByTestId("account-session-requests-all")).toBeTruthy(),
    );
    await fireEvent.press(ui.getByTestId("account-session-requests-all"));
    await waitFor(() =>
      expect(ui.getByTestId("session-service-requests-screen")).toBeTruthy(),
    );
