...
function GuestProviders({
  ...
  client?: QueryClient;
  initialContext?: ActiveReservationContext | null;
  initialSession?: GuestAuthSession | null;
}>) {
  return (
    <QueryClientProvider client={client}>
      <GuestAuthSessionProvider initialSession={initialSession}>
        <ActiveReservationContextProvider
          initialActiveReservationContext={initialContext}
        >
          {children}
        </ActiveReservationContextProvider>
      </GuestAuthSessionProvider>
...
function LogoutStateProbe() {
  ...
  return (
    <View>
      <Text testID="guest-navigation-session-probe">
        {session?.accountId ?? "none"}
      </Text>
      <Text testID="guest-navigation-context-probe">
        {activeReservationContext
          ? activeReservationContext.reservationId +
            "/" +
            activeReservationContext.reservationStayId
          : "none"}
      </Text>
    </View>
  );
...
function createLogoutDrawerLayout(
  client: QueryClient,
  initialSession: GuestAuthSession,
  initialContext: ActiveReservationContext,
) {
...
function createDirtyRoute(onDiscard: () => void) {
  return function DirtyRoute() {
    ...
    return (
      <View>
        <GuestRootHeader title="Cambios" />
        <Text>Formulario con cambios</Text>
      </View>
    );
  };
}
...
function installAndroidBackHandler() {
  ...
  const addEventListener = jest
    ...
    .mockImplementation((_event, nextHandler) => {
  ...
  return {
    addEventListener,
    trigger: () =>
      [...handlers].reverse().some((handler) => handler({} as never) === true),
    restore: () => {
...
describe("Guest Navigation Shell", () => {
  it("declares only Inicio, Servicios, Valet and Hotel in the approved tab order", () => {
    expect(guestNavigationTabs).toEqual([
      {
        id: "home",
        icon: { android: "home", ios: "house.fill", web: "home" },
        label: "Inicio",
        basePath: "/account",
        disabled: false,
      },
    ...
    expect(guestNavigationDrawerPrimaryLink).toEqual({
      icon: { android: "person", ios: "person.fill", web: "person" },
      label: "Perfil",
      path: "/account/profile",
    });
    expect(guestNavigationDrawerLinks.map((link) => link.label)).toEqual([
      "Perfil",
      "Inicio",
      "Mis servicios",
      "Check-out",
    ...
    expect(
      guestNavigationDrawerSections.map((section) => section.label),
    ).toEqual([
      "CUENTA",
      "ESTANCIA",
      "BENEFICIOS",
  ...
  it("resolves active tabs from root paths, child paths, and routes outside the shell", () => {
    ...
    expect(resolveActiveGuestNavigationTab("/services/selection")).toBe(
      "services",
    );
  ...
  it("renders the four enabled V3 tabs and navigates only from a non-active tab", async () => {
    ...
    const rendered = await render(
      <GuestNavigationTabs onReplace={onReplace} pathname="/account" />,
    );
    const tabs = rendered.getAllByRole("tab");

    expect(
      rendered.getByLabelText("Navegación principal de huésped").props
        .accessibilityRole,
    ).toBe("tablist");
    ...
    expect(
      rendered.getByTestId("guest-navigation-tab-icon-services"),
    ).toBeTruthy();
    expect(
      rendered.getByTestId("guest-navigation-tab-icon-valet"),
    ).toBeTruthy();
    expect(
      rendered.getByTestId("guest-navigation-tab-icon-hotel"),
    ).toBeTruthy();
  ...
  it.each([
    ...
    "marks %s with its matching tab selected",
    async (pathname, active) => {
      const rendered = await render(
        <GuestNavigationTabs pathname={pathname} />,
      );

      guestNavigationTabs.forEach((tab) => {
        expect(
          rendered.getByTestId(`guest-navigation-tab-${tab.id}`).props
            .accessibilityState,
        ).toEqual({ disabled: false, selected: tab.id === active });
      });
    },
  );
  ...
  it("opens the current section and keeps a single drawer section expanded", async () => {
    ...
    await waitFor(() =>
      expect(
        rendered.getByTestId("guest-navigation-drawer-panel"),
      ).toBeTruthy(),
    );
    expect(rendered.getByText("Menú")).toBeTruthy();
    expect(rendered.getByText("Navega por tu estancia")).toBeTruthy();
    expect(
      rendered.getByTestId("guest-navigation-drawer-section-services").props
        .accessibilityState,
    ).toEqual({ expanded: true });
    expect(
      rendered.getByTestId("guest-navigation-drawer-section-services-chevron")
    ...
    expect(
      rendered.getByTestId("guest-navigation-drawer-section-benefits-chevron")
        .props.children.props.name,
    ).toEqual({
    ...
    expect(
      rendered.getByTestId("guest-navigation-drawer-section-services-icon"),
    ).toBeTruthy();
    expect(
      rendered.getByTestId("guest-navigation-drawer-link-servicios").props
        .accessibilityState,
    ...
    await fireEvent.press(
      rendered.getByTestId("guest-navigation-drawer-section-benefits"),
    );
    expect(
      rendered.getByTestId("guest-navigation-drawer-section-benefits").props
        .accessibilityState,
    ).toEqual({ expanded: true });
    expect(
      rendered.getByTestId("guest-navigation-drawer-section-benefits-chevron")
    ...
    expect(
      rendered.getByTestId("guest-navigation-drawer-link-rewards"),
    ).toBeTruthy();
    await act(async () => {
    ...
    await waitFor(() =>
      expect(
        rendered.queryByTestId("guest-navigation-drawer-panel"),
      ).toBeNull(),
    );
  });
  ...
  it("closes the shared drawer from its backdrop and does not leave a second drawer mounted", async () => {
    ...
    expect(
      rendered.getAllByTestId("guest-navigation-drawer-panel"),
    ).toHaveLength(1);
    await act(async () => {
      fireEvent.press(
        rendered.getByTestId("guest-navigation-drawer-backdrop", {
  ...
  it("keeps Factura visible but disabled until a checkout snapshot exists", async () => {
    ...
    const invoice = rendered.getByTestId(
      "guest-navigation-drawer-link-factura",
    );
    expect(invoice.props.accessibilityState).toMatchObject({
      disabled: true,
      selected: false,
    });
    expect(invoice.props.accessibilityHint).toBe(
      "Disponible después del check-out.",
    );
    await act(async () => {
      fireEvent.press(invoice);
    });
  ...
  it("enables Factura and closes the drawer when a checkout snapshot exists", async () => {
    ...
    await waitFor(() =>
      expect(rendered.getByTestId("profile-route")).toBeTruthy(),
    );
  ...
  it("navigates directly from the Benefits drawer entry to Promotions", async () => {
    ...
    await act(async () => {
      fireEvent.press(
        rendered.getByTestId("guest-navigation-drawer-link-promociones"),
      );
    });
    await waitFor(() =>
      expect(rendered.getByTestId("promotions-route")).toBeTruthy(),
    );
  });
  ...
  it("navigates from the first Perfil drawer action to Profile", async () => {
    ...
    await act(async () => {
      fireEvent.press(
        rendered.getByTestId("guest-navigation-drawer-section-account"),
      );
    });
    await act(async () => {
      fireEvent.press(
        rendered.getByTestId("guest-navigation-drawer-link-perfil"),
      );
    });
    await waitFor(() =>
      expect(rendered.getByTestId("profile-route")).toBeTruthy(),
  ...
  it("shows Cerrar sesión outside the accordion sections and only after confirmation clears the Guest session, context, and query cache", async () => {
    ...
    queryClient.setQueryData(["guest", "previous-session"], {
      guest: "previous",
    });
    ...
    const initialContext = {
      ...
      reservationStayId: "stay-primary",
    };
    const rendered = await renderRouter(
      {
        _layout: createLogoutDrawerLayout(
          queryClient,
          initialSession,
          initialContext,
        ),
    ...
    expect(
      rendered.getByTestId("guest-navigation-session-probe").props.children,
    ).toBe("guest-account-primary");
    expect(
      rendered.getByTestId("guest-navigation-context-probe").props.children,
    ).toBe("reservation-primary/stay-primary");
    ...
    expect(
      within(body).queryByTestId("guest-navigation-drawer-logout"),
    ).toBeNull();
    expect(footer).toBeTruthy();
    expect(rendered.getByTestId("guest-navigation-drawer-logout")).toBeTruthy();
    expect(rendered.getByTestId("guest-navigation-drawer-footer")).toBeTruthy();
    expect(
      rendered.getByTestId("guest-navigation-drawer-logout").props
        .accessibilityLabel,
    ).toBe("Cerrar sesión");

    await fireEvent.press(
      rendered.getByTestId("guest-navigation-drawer-logout"),
    );
    expect(rendered.getByTestId("guest-navigation-logout-modal")).toBeTruthy();
    expect(rendered.getByText("¿Cerrar sesión?")).toBeTruthy();
    expect(
      rendered.getByText("Volverás a la pantalla de inicio de sesión."),
    ).toBeTruthy();
    await fireEvent.press(
      rendered.getByTestId("guest-navigation-logout-modal-cancel"),
    );
    ...
    expect(queryClient.getQueryData(["guest", "previous-session"])).toEqual({
      guest: "previous",
    });
    ...
    await fireEvent.press(
      rendered.getByTestId("guest-navigation-logout-modal-confirm"),
    );
    expect(
      rendered.getByTestId("guest-navigation-session-probe").props.children,
    ).toBe("none");
    ...
    expect(
      queryClient.getQueryData(["guest", "previous-session"]),
    ).toBeUndefined();
  ...
  it("requires the existing dirty-discard confirmation before opening logout confirmation", async () => {
    ...
    const rendered = await renderRouter(
      {
        _layout: createLogoutDrawerLayout(
          queryClient,
          { accountId: "guest-account-primary" },
          {
            reservationId: "reservation-primary",
            reservationStayId: "stay-primary",
          },
    ...
    expect(rendered.queryByTestId("guest-navigation-logout-modal")).toBeNull();
    await fireEvent.press(
      rendered.getByTestId("guest-navigation-discard-modal-confirm"),
    );
  ...
  it.each(["/services", "/valet", "/hotel"] as const)(
    "uses Android Back from %s to replace with /account",
    async (path) => {
      const back = installAndroidBackHandler();
      const replaceSpy = jest
        .spyOn(router, "replace")
  ...
  it("uses Android Back to close an open drawer before any root navigation", async () => {
    ...
    const replaceSpy = jest
      ...
      .mockImplementation(() => undefined as never);
    const rendered = await renderRouter(
      { _layout: DrawerLayout, account: AccountRoute },
      { initialUrl: "/account" },
    );
  ...
  it("prepares replace navigation for Inicio and keeps its current route as a no-op", () => {
    ...
    expect(
      getGuestNavigationTabPressHandler(home, "/account", onReplace),
    ).toBeUndefined();
