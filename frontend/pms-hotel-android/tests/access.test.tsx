...
async function renderAccess(
  ...
  onLinked: () => void = jest.fn(),
) {
  function AccessRoute() {
    return (
      <QueryClientProvider client={createQueryClient()}>
        <ActiveReservationContextProvider>
          <AccessScreen onLinked={onLinked} service={service} />
        </ActiveReservationContextProvider>
      </QueryClientProvider>
    );
  }
...
async function fillValidAccess(
  screen: Awaited<ReturnType<typeof renderAccess>>["screen"],
) {
  await fireEvent.changeText(
    screen.getByTestId("access-reservation-code"),
    "HB-2026-004281",
  );
  await fireEvent.changeText(
    screen.getByTestId("access-email"),
    "ana@example.com",
  );
}
...
describe("Access / Vincular reserva", () => {
  it("renders the direct /access route outside Guest navigation", async () => {
    function AccessRoute() {
      return (
        <QueryClientProvider client={createQueryClient()}>
          <ActiveReservationContextProvider>
            <AccessScreen service={new MockAccessService()} />
          </ActiveReservationContextProvider>
        </QueryClientProvider>
      );
    }
    ...
    await waitFor(() =>
      expect(screen.getByTestId("access-screen")).toBeTruthy(),
    );
  });
  ...
  it("renders precisely the approved Access form without Guest V3 navigation", async () => {
    ...
    expect(screen.getByTestId("access-reservation-code").props.maxLength).toBe(
      32,
    );
  ...
  it("keeps mismatch typed internally while showing only generic verification copy", async () => {
    ...
    await fireEvent.press(screen.getByTestId("access-submit-button"));
    await waitFor(() =>
      expect(screen.getByTestId("access-verification-failed")).toBeTruthy(),
    );
    expect(screen.getByText("No pudimos verificar los datos")).toBeTruthy();
    expect(
      screen.getByText(
        "Revisa el código de reserva y el correo e inténtalo de nuevo.",
      ),
    ).toBeTruthy();
    expect(screen.queryByText("Reserva no encontrada")).toBeNull();
    expect(screen.queryByText("Correo no registrado")).toBeNull();
    expect(screen.getByTestId("access-reservation-code").props.value).toBe(
      "HB-2026-004281",
    );
    expect(screen.getByTestId("access-email").props.value).toBe(
      "ana@example.com",
    );
    await fireEvent.changeText(
      screen.getByTestId("access-reservation-code"),
  ...
  it("keeps fields and allows manual retry for generic and offline failures", async () => {
    ...
    await fireEvent.press(
      genericResult.screen.getByTestId("access-submit-button"),
    );
    await waitFor(() =>
      expect(genericResult.screen.getByTestId("access-error")).toBeTruthy(),
    );
    await fireEvent.press(genericResult.screen.getByText("Reintentar"));
    await waitFor(() =>
      expect(genericResult.onLinked).toHaveBeenCalledTimes(1),
    );
    ...
    await fireEvent.press(
      offlineResult.screen.getByTestId("access-submit-button"),
    );
    await waitFor(() =>
      expect(offlineResult.screen.getByTestId("access-offline")).toBeTruthy(),
    );
    await fireEvent.press(offlineResult.screen.getByText("Reintentar"));
    await waitFor(() =>
      expect(offlineResult.onLinked).toHaveBeenCalledTimes(1),
    );
  });
