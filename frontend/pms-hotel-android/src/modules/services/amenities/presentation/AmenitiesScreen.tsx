...
export function AmenitiesScreen({ service, stayService, nowMs }: AmenitiesScreenProps) {
  ...
  const goBack = () => { if (cartVisible && step === 'SCHEDULE') { setCartVisible(false); setStep('CATALOG'); return; } if (cartVisible) { setCartVisible(false); setStep('CATALOG'); return; } requestExit(edited ? returnFromEdit : () => router.dismissTo('/services')); };
