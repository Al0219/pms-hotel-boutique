...
export function AmenitiesScreen({ service, stayService, nowMs = Date.now }: AmenitiesScreenProps) {
  ...
  return <View style={styles.screen} testID="amenities-screen"><SafeAreaView edges={['top']}><View style={styles.header}><Pressable accessibilityLabel="Volver" accessibilityRole="button" onPress={goBack} style={styles.back} testID="amenities-back"><Text style={styles.backLabel}>←</Text></Pressable><Text accessibilityRole="header" style={styles.title}>Amenidades</Text>{step === 'CATALOG' ? <Pressable accessibilityLabel="Abrir carrito" accessibilityRole="button" onPress={() => setStep('CART')} style={styles.cart} testID="amenities-cart-button"><Text>🛒 <Text testID="amenities-cart-badge">{totalQuantity(items)}</Text></Text></Pressable> : null}</View></SafeAreaView>
