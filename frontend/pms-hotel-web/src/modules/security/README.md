...
Security-session presentation and sensitive-action boundaries.
...
May consume intentional public APIs from: auth, permissions, privacy.
...
Implemented layers for the explicitly authorized task IMP-WEB-0902 / IMP-WEB-0903:
...
Cross-module dependencies use `@/modules/security`, never paths such as `@/modules/security/service/*`, `dtos/*`, `mappers/*`, `hooks/*`, or `components/*`.
...
- Routes: /seguridad/sesiones y /seguridad/mfa.
- Public UI API: SessionsPage, MfaPage, SecurityFrame, DemoLoadState, DemoFeedback.
