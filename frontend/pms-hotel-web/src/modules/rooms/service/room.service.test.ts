...
describe("listRooms", () => {
  it("uses the supplied approved endpoint and preserves the property scope", async () => {
    mockServer.use(http.get("http://pms.test/contract/rooms", ({ request }) => {
      expect(new URL(request.url).searchParams.get("propertyId")).toBe("GT-HB-01");
      return HttpResponse.json({ rooms: [] });
    }));

    await expect(
      listRooms({ endpoint: "http://pms.test/contract/rooms", propertyId: "GT-HB-01" }),
    ).resolves.toEqual({ rooms: [] });
  });

  it("returns technical HTTP errors to the hook", async () => {
    mockServer.use(http.get("http://pms.test/contract/rooms", () => HttpResponse.text(null, { status: 503 })));
